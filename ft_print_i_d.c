...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/01/26 19:30:39 by kmeeseek          #+#    #+#             */
/*   Updated: 2021/01/30 04:55:11 by kmeeseek         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
					...
					int ap_arg)
{
	...
	ap_arg = ft_ngt_arg(fplist, ap_arg, 1);
	if (fplist->ngt == 0)
	{
		ft_while(fplist, 1);
		ap_arg = ft_ngt_arg(fplist, ap_arg, 2);
					...
					int ap_arg)
{
	...
	{
		fplist->width = fplist->width - num_len;
		ap_arg = ft_ngt_arg(fplist, ap_arg, 3);
		if (ap_arg == 0 && fplist->precision != 1)
			fplist->width += 1;
		ft_while(fplist, 3);
		if (ap_arg == 0 && fplist->precision != 1)
...
static void			ft_smaller_p_w(t_struct *fplist, int ap_arg)
{
